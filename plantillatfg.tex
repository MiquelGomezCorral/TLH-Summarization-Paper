...
Se han utilizado LLMs para la interpretación inicial de los resultados con el objetivo de tener una primera idea. Luego, se han analizado de forma detallada y se han llegado a conclusiones propias. La redacción se ha hecho a mano, usando esto modelos como apoyo en la corrección del texto y el formato.
...
En este dataset, el panorama cambia significativamente. Ya que ningún modelo ha sido entrenado específicamente en CLASum, los resultados reflejan mejor la capacidad de generalización de cada método.
...
El cambio más relevante ocurre en los modelos \textit{Sequence2Sequence}. Flan-T5 obtiene los mejores resultados en casi todas las métricas, superando a BART y Pegasus. Ahora, viendo los valores, las diferencias no son tan pronunciadas como en el dataset anterior, por lo que no se puede mensopreciar la capacidad de generalización de Bart y Pegasus. Todos obtienen resultados competitivos, pero Flan-T5 consigue los mejores valores en Relevancia (BertScore-F, METEOR) y en Consistencia (R-3, BertScore-R), mientras que Pegasus gana en Coherencia y Fluidez (R-4 y CHRF).

Finalmente, los modelos causales (GPT2 y Llama3) tienen resultados similares a los del datase anterior. Siguen teniendo un rendimiento inferior al de los demás métodos, aunque en este caso, Llama3 obtiene mejores resultados que GPT2 en todas las métricas. Por lo mencionado anteriormente, siguen en desventaja debido a su naturaleza generativa.


\subsection{Comentario sobre los oráculos}
En ambos datases, los métodos que no son muy abstactivos (extractivos y \textit{Sequence2Sequence}) se acercan bastante a los oráculos, especialmente en las métricas de Relevancia y Consistencia. también, si nos fijamos en el método naive LEAD-3, vemos que obtiene resultados decentes e incluso mejores que los demás métodos.

Con todo esto, llegamos a la conclusión de que ambos datasets tienen un nivel de abstractividad bajo, y que los resúmenes, son en gran parte extractivos. Los oráculos marcan un límite alto, y el hecho de que los métodos extractivos y \textit{Sequence2Sequence} (que no son muy abstractivos) se acerquen tanto a ellos confirman esta teoría. Los resultados del LEAD-3 también apoyan esta idea.
...
Comparando el comportamiento de los modelos entre los distintos datasets, observamos patrones muy diferenciados. Los métodos extractivos demuestran una gran estabilidad en todos los aspectos, manteniendo métricas consistentes tanto en CNN/DailyMail como en CLASum. Por el contrario, los modelos abstractivos Sequence2Sequence BART y Pegasus, empeoran bastante del primer corpus al segundo, su rendimiento en la relevancia y coherencia disminuyen notablemente. Ahora, el modelo Flan-T5, que no había sido finetuneado en ninguno de estos datasets, mejora en CLASum. Por último, los modelos causales GPT-2 y Llama-3 mantienen un rendimiento bajo en ambos datasets, con ligeras variaciones pero sin cambios significativos.



En este punto, si tuviéramos que elegir un método para trabajar en ambos datasets, elegiríamos un método abstractivo basado en modelos Sequence2Sequence. Entre ellos, es complicado decidir cual de los tres, sin embargo, nos quedaríamos con Pegasus. 
...
Viendo la tabla con las correlaciones de "Kendall tau", se apricia que existe una correlación negativa evidente entre el rendimiento y el nivel de abstractividad. Como se ha apuntado durante el análisis, esto es debido al diseño de las métricas automáticas, que se basan en referencias extractivas. Si los resúmenes de referencia se vuelven muy abstractos, se alejan del texto fuente, lo que penaliza a cualquier modelo que se ciña al texto original.

Todos los métodos que presentan una correlación negativa relevante, son los extractivos y los modelos \textit{Sequence2Sequence}, mientras que los causales, tienen una correlación cercana a cero. 

Tal como se adelantó en el análisis de rendimiento, los modelos causales (GPT-2, Llama-3) presentan una correlación significativamente menor (cercana a cero) en valor absoluto. 
